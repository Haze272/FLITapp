...
public class ResponsesViewModel extends ViewModel implements DisplayTasksViewModel {
    ...
    public MutableLiveData<ArrayList<Task>> getResponseTasks(View v) {
        if (currentTask == null) {
            currentTask = new MutableLiveData<ArrayList<Task>>();
        }
        currentTask.setValue(userRepository.getResponseTasks(1));

        return currentTask;
    }

    public boolean isHasResponseTasks() {
        if (userRepository.getResponseTasks(1) == null) {
            return false;
        } else {
            return true;
        }
